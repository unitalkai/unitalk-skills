...
- NOT for scanned/image-only PDFs (use `mistral-ocr`) and NOT for pixel-perfect HTML-to-PDF rendering (use a headless browser).
...
1. **Inspect first.** Run `pdf_read.py file.pdf --meta`. Check `encrypted` (if true, decrypt first with `pdf_secure.py --decrypt`) and `likely_scanned_pages`. If pages are image-only, load the `mistral-ocr` skill and OCR via Hermes-web `POST /api/ocr` with a fetchable `documentUrl` (or hosted `imageUrl` per page). Optionally export previews with `pdf_page_image.py --pages <scanned> --dpi 300 --out-dir imgs/` — do not report empty text as "no content".
...
- **Scanned PDFs**: empty `extract_text()` plus page images means there is no text layer. Route to `mistral-ocr`; do not fabricate text.
